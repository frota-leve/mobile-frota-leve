...
class RaceService {
  ...
  static async getRaces(body: { token: string; page?: number; size?: number }) {
    const route = `${baseURL}/api/races/me?page=${body.page || 0}&size=${
      body.size || 10
    }`;
  ...
  static async getRaceById(id: string, token: string) {
    const route = `${baseURL}/api/races/${id}`
